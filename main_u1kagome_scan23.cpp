...
// Here, we want to scan the ratios r2/r3 in a reasonable range
...
//  wf->pars->xi[1] = ((double)atoi(argv[7]))/100.;
  ...
  int rtot = atoi(argv[ 8]); //total xi2 + |xi3|
  int r0   = atoi(argv[ 9]); // starting |xi3|
  int step = atoi(argv[10]); // stepsize for |xi3|
  int sgn  = atoi(argv[11])==0 ? 1 : -1 ;  //sign of xi3 to scan
  ...
  wf->pars->xi[0] = ((double)atoi(argv[12]))/100.; //xi1 is fixed
  ...
  if( sgn == -1 ) {
    cout << "Scanning with xi3 = -" << r0 << " ... -" << rtot << ", step " << step << "." << endl;
  } else {
    cout << "Scanning with xi3 = " << r0 << " ... " << rtot << ", step " << step << "." << endl;
  }
